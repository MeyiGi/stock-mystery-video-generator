...
class YearInReviewAnimator:
    ...
    def create_animation(self, data, stock_name, year, output_path,
        ...
        if isinstance(year_data_daily, pd.DataFrame):
            year_data_daily = year_data_daily.squeeze()  # Convert single-column DataFrame to Series
        ...
        def update(frame_index):
            ...
            last_val = cur_data.iloc[-1]

            # Ensure it's a scalar value, not a Series
            if isinstance(last_val, pd.Series):
                cur_price = float(last_val.iloc[0])
            elif hasattr(last_val, 'item'):
                cur_price = float(last_val.item())
            else:
                cur_price = float(last_val)
            # ========================================

            # Avoid passing Series to matplotlib
            y_values = (
                cur_data.values
                if hasattr(cur_data, "values")
                else np.array(cur_data)
            )

            line.set_data(cur_data.index, y_values)
            head.set_data([cur_date], [cur_price])

            date_label.set_position((
                cur_date,
                cur_price + float(ax.get_ylim()[1]) * 0.05
            ))
